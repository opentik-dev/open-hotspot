...
grep -F 'dispatcher.context.authtoken' "$controller" >/dev/null
...
grep -F 'backup.sh' "$controller" >/dev/null
grep -F 'multipart/form-data' "$backup" >/dev/null
grep -F 'Validate archive' "$backup" >/dev/null
grep -F 'setup_base' "$setup" >/dev/null
grep -F 'ubus.connect' "$setup" >/dev/null
grep -F 'client_network' "$setup" >/dev/null
grep -F 'client_router_access' "$setup" >/dev/null
