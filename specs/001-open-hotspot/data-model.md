...
| `renewed_at` | TEXT | nullable | Start of an administrative quota reset inside the current calendar period |
...
requires the explicit audited LuCI/RPC operation; it is refused while the
device has a live session. An authentication attempt may never silently move a
device to another account.
...
Schema version 3 adds `policy_period_start`. Schema version 4 adds
`accounts.renewed_at`. The cycle job uses the policy marker to refresh
...
An account renewal does not delete historical usage. It records `renewed_at`;
the effective current period starts at that UTC instant and ends at the
existing calendar boundary. At the next calendar boundary normal period
calculation resumes. Active sessions are sent through the verified deauth
adapter so reconnecting devices receive the new policy; close callbacks remain
responsible for their final accounting.
