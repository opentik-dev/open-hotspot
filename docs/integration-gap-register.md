...
| INT-022 | Reboot restore is configured as enabled but no automatic cycle/restore runs after boot. | The Open-HotSpot init service was inactive and its cron entries were absent after a prior installation path, so manual restore worked but scheduled reconciliation did not. | r79 enables/starts the manager service when local FAS is activated and repairs already-enabled installations during live upgrade; offline image roots are not mutated. | Reproduced on r78; r79 target upgrade created the service/cron entries and automatic restore passed after reboot and first client traffic. |
