...
1.2.0-r60`. r60 is the current candidate with router-access isolation and
runtime readiness recovery; it
...
| High | FAS keeps a stale upstream/WAN address after the Internet source changes. | Medium | High | `fasremoteip` and `fasremotefqdn` remain unset, `gatewayfqdn=status.client`, and a client on Open-HotSpot `br-lan` completes the portal after changing upstream connectivity. | Engineering + field validation | Code guard in r48 — client proof open |
...
The delivery artifact and the exact factory-reset acceptance procedure are
